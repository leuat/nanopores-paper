...
We now perform a parameter fitting analysis on realistic simulated nanoporous media. Using LAMMPS \cite{plimpton1995fast}, we simulate a $100^3\AA$  box of 65K SiO2 silica particles at an initial temperature of $4500 K$, using the Vashishta-potential. After an initial period, we expand the box to $159^3 \AA$ and start cooling until quenching is achieved at about $300K$. The resulting nanoporous media is depicted on the right side in figure \ref{fig:porous_vs_model}, with a porosity of $75\%$. \todo{Skrive mer om lagingen?}
