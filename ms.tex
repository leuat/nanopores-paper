...
% The rapid development of improved experimental means of three dimensional characterization allows us to scan and find the three dimensional structures of many types of porous media.
% On small scales, FIB-SEM methods open for small scale characterization - almost down to the atomic scale
% Modeling of ensembles requires many realizations of statistically similar samples.
% Characterization and generation of synthetic samples that are statistically similar.
% Smaller scales, the need to fill in atomic-scale details from characterization methods
% Procedurally generated materials.

% May generate porous media using physically motivated methods or statistical methods. This has been developed as a method across scale, from the advanced geomorphological methods used to render past geological processes for realistic reservoirs to
% Examples of physical methods - ingrain etc, filling in grains, growing grains. 
% Examples of more statistical methods - percolation, random placement of grains or pores,
% Pore-neck networks as a model for porous media - not equally tunable

% Thus, there is a need for effective methods to generate many samples.

% Recent growth in advanced random noise models opens for new ways to generate structured noise with statistical similarities to real samples. While not based on a physical model.
% Developed in many areas - and shows great promise, but has not been systematically applied to generate random porous samples with given statistical properties.

% Here we develop ...

% Need efficient methods to develop structures that are statistically similar to a given experimental structure
% Extensions and boundaries - non-limited models open for 
